...
describe("queryOpinions", () => {
  ...
  it("filters by termYear", async () => {
    ...
    expect(rows).toHaveLength(
      SEED_OPINIONS.filter((r) => r.term_year === termYear).length,
    );
  });
  ...
  it("filters by docket", async () => {
    ...
    expect(rows).toHaveLength(
      SEED_OPINIONS.filter((r) => r.docket === docket).length,
    );
  });
  ...
  it("combines opinionType and termYear filters", async () => {
    ...
    expect(rows).toHaveLength(
      SEED_OPINIONS.filter(
        (r) => r.opinion_type === opinionType && r.term_year === termYear,
      ).length,
    );
  });
